...
cl <- read.csv(here("raw_data", "Country_level_data.csv"), strip.white = T, stringsAsFactors = F) %>% 
  ...
  filter(!(reference == "Mathieu et al., 2014" & n_fads == 400),
         !str_detect(x_event, "\\*"))
```
...
```{r, fig.height = 6.75, fig.width = 4.5}
cowplot::plot_grid(bef, aft, labels = "AUTO", ncol = 1)
```
...
(timeline <- cl %>% 
  ...
  mutate(x_event = case_when(x_event == "Average adoption" ~ "Adoption",
                             ...
                             x_event == "Proposed public FAD" ~ "Introduction",
                             T ~ x_event),
         event = fct_relevel(x_event, c("Introduction", "Adoption")),
